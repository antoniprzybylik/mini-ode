...
class TestSolverWarnings:
    ...
    def test_euler_divergence_warning_emitted(self):
        ...
        traced_f = torch.jit.trace(f, (torch.tensor(0.0), torch.tensor([0.0, 0.0])))

        solver = mini_ode.EulerMethodSolver(step=1.0)

        with pytest.warns(RuntimeWarning, match="solution norm exceeded"):
            xs, ys = solver.solve(traced_f, (0.0, 15.0), torch.tensor([1.5, 0.0]))
    ...
    def test_warnings_suppressed_no_record(self):
        ...
        with warnings.catch_warnings(record=True) as ws:
            ...
            )  # Record other warnings

            solver.solve(traced_f, (0.0, 15.0), torch.tensor([1.5, 0.0]))
    ...
    def test_warning_ignore_vs_always_comparison(self):
        ...
        with warnings.catch_warnings(record=True) as ws_always:
            ...
            solver.solve(traced_f, (0.0, 15.0), torch.tensor([1.5, 0.0]))
            always_count = len(
                [w for w in ws_always if issubclass(w.category, RuntimeWarning)]
            )
        ...
        with warnings.catch_warnings(record=True) as ws_ignore:
            ...
            solver.solve(traced_f, (0.0, 15.0), torch.tensor([1.5, 0.0]))
            ignore_count = len(
                [w for w in ws_ignore if issubclass(w.category, RuntimeWarning)]
            )
