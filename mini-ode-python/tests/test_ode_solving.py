...
def test_euler_solver_exp_1d():
    ...
    f_traced = torch.jit.trace(f, (torch.tensor(0.0), torch.tensor([0.0])))

    solver = mini_ode.EulerMethodSolver(step=0.01)
    xs, ys = solver.solve(f_traced, (0.0, 1.0), torch.tensor([1.0]))

    assert xs.shape == (101,)
    assert ys.shape == (101, 1)
    assert (
        (ys - torch.exp(torch.linspace(0.0, 1.0, 101).unsqueeze(1))).abs() <= 0.05
    ).all()
...
def test_euler_solver_exp_2d():
    ...
    f_traced = torch.jit.trace(f, (torch.tensor(0.0), torch.tensor([0.0, 0.0])))

    solver = mini_ode.EulerMethodSolver(step=0.01)
    xs, ys = solver.solve(f_traced, (0.0, 1.0), torch.tensor([1.0, 1.0]))

    assert xs.shape == (101,)
    assert ys.shape == (101, 2)
    assert (
        (
            ys - torch.exp(torch.linspace(0.0, 1.0, 101).unsqueeze(1).repeat([1, 2]))
        ).abs()
        <= 0.05
    ).all()
...
def test_implicit_euler_solver_exp_1d():
    ...
    solver = mini_ode.ImplicitEulerMethodSolver(
        step=0.01, optimizer=mini_ode.optimizers.Newton(max_steps=3)
    )
    xs, ys = solver.solve(f_traced, (0.0, 1.0), torch.tensor([1.0]))
...
def test_glrk4_solver_exp_1d():
    ...
    solver = mini_ode.GLRK4MethodSolver(
        step=0.01, optimizer=mini_ode.optimizers.Newton(max_steps=3)
    )
    xs, ys = solver.solve(f_traced, (0.0, 1.0), torch.tensor([1.0]))
